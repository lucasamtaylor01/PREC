...
def resolve_eq_onda(
    ...
    if psi is not None:
        ...
        U[1, 1:-1] = (
            (c**2 * lambda_val**2 / 2) * (U[0, :-2] + U[0, 2:])
            + (1 - c**2 * lambda_val**2) * U[0, 1:-1]
            + tau * psi(x[1:-1])
        )
        U[1, 0] = aplica_cond_contorno(
            U, t, 1, 0, cont_esq, tipo_cont_esq, h, ordem_neumann
        )
        U[1, -1] = aplica_cond_contorno(
            U, t, 1, M, cont_dir, tipo_cont_dir, h, ordem_neumann
        )

    # Esquema explícito para demais passos
    for n in range(1, N):
        U[n + 1, 1:-1] = (
            c**2 * lambda_val**2 * (U[n, :-2] + U[n, 2:])
            + 2 * (1 - c**2 * lambda_val**2) * U[n, 1:-1]
            - U[n - 1, 1:-1]
        )
        U[n + 1, 0] = aplica_cond_contorno(
            U, t, n + 1, 0, cont_esq, tipo_cont_esq, h, ordem_neumann
        )
        U[n + 1, -1] = aplica_cond_contorno(
            U, t, n + 1, M, cont_dir, tipo_cont_dir, h, ordem_neumann
        )
